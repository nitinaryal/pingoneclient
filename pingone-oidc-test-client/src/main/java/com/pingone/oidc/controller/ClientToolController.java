...
import com.pingone.oidc.client.ClientIntegrationSummary;
...
public class ClientToolController {
    ...
    public ClientToolController(
            ...
            PingOneClientIntegrationRegistry integrationRegistry,
    ...
    @GetMapping(value = "/tool/api/integrations", produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public List<ClientIntegrationSummary> integrations() {
        return integrationRegistry.all().stream().map(ClientIntegrationSummary::from).toList();
    }
